...
### ヘッダーQRコード
```
HDR|v2|total:20|size:5234|hash:a1b2c3d4e5f6|enc:GZIP|bytes:1820|chunk:330|fec:10x2
```

### データQRコード
```
DAT|v2|sid:a1b2c3d4e5f6|seq:1|data:Base64データ|crc:12345678
```

データQRのグループごとに2枚の復元用QR（`PAR`）を追加し、同一グループ内で最大2枚を取りこぼしても復元できる。対応ブラウザではテキストをgzip圧縮し、圧縮効果がない場合はUTF-8のまま送信する。

送信モードにより、1枚あたりのデータ量と復元用QRの比率を変更する。初期値の「読み取り優先」は8枚ごとに2枚、「バランス」は10枚ごとに2枚、「枚数優先」は16枚ごとに2枚の復元用QRを追加する。

## 送信側の主要処理
1. テキストをUTF-8エンコード
2. 有効な場合はgzip圧縮し、330バイト単位に分割
3. ヘッダー、データ、復元用QRコードを生成
4. 設定された間隔で一度ずつ順次表示
